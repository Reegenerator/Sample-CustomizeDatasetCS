namespace System.Windows.Forms.Design
{
    ...
    public class AxWrapperGen
    {
        ...
        private static string Compile(AxImporter importer, CodeNamespace ns, string[] refAssemblies, DateTime tlbTimeStamp, Version version)
        {
            ...
            assemblyCustomAttributes.Add(new CodeAttributeDeclaration("System.Windows.Forms.AxHost.TypeLibraryTimeStamp", new CodeAttributeArgument[] { new CodeAttributeArgument(new CodePrimitiveExpression(tlbTimeStamp.ToString(CultureInfo.InvariantCulture))) }));
        ...
        internal static string GenerateWrappers(AxImporter importer, Guid axClsid, Assembly rcwAssem, string[] refAssemblies, DateTime tlbTimeStamp, bool ignoreRegisteredOcx, out string assem)
        {
            ...
            try
            {
                ...
                {
                    if ((ignoreRegisteredOcx && IsTypeComObject(types[i])) || (!ignoreRegisteredOcx && IsTypeActiveXControl(types[i])))
                    {
                        flag = true;
                        if (ns == null)
                        {
        ...
        private static bool IsTypeComObject(System.Type type)
        {
            ...
            private static CodeExpression GetDefaultExpressionForInvoke(MethodInfo method, AxParameterData parameterInfo)
            {
                ...
                if (rawDefaultValue == Missing.Value)
                {
                    if (!parameterBaseType.IsPrimitive)
                    {
                        ...
                        {
                            rawDefaultValue = 0;
                            FieldInfo[] fields = parameterBaseType.GetFields(BindingFlags.Public | BindingFlags.Static);
                            if ((fields.Length > 0) && !Enum.IsDefined(parameterBaseType, 0))
                            {
                                rawDefaultValue = fields[0].GetRawConstantValue();
                            }
                        }
                    }
                    else
